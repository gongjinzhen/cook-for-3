...
router.post('/', auth, upload.single('cover'), (req, res) => {
  const { name, tags, nutrition, ingredients, steps, is_signature } = req.body;
  const cover = req.file ? '/uploads/' + req.file.filename : '';
  const result = db.prepare('INSERT INTO recipes (user_id, name, cover, tags, nutrition, ingredients, steps, is_signature) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(req.user.id, name, cover, tags || '', nutrition || '', ingredients || '', steps || '', is_signature ? 1 : 0);
...
router.put('/:id', auth, upload.single('cover'), (req, res) => {
  ...
  db.prepare("UPDATE recipes SET name=?, cover=?, tags=?, nutrition=?, ingredients=?, steps=?, is_signature=?, updated_at=CURRENT_TIMESTAMP WHERE id=?").run(name || recipe.name, cover, tags ?? recipe.tags, nutrition ?? recipe.nutrition, ingredients ?? recipe.ingredients, steps ?? recipe.steps, is_signature !== undefined ? (is_signature ? 1 : 0) : recipe.is_signature, req.params.id);
